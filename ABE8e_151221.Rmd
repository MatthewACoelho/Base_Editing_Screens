...
scale_colour_manual(values=c("black", "springgreen3", "yellow", "orange","royalblue1", "grey65", "purple")) +
...
scale_shape_manual(values=c(15, 16, 17, 18)) +
scale_colour_manual(values=c("black", "yellow", "orange", "royalblue1", "red2", "grey65", "purple")) +
geom_point(alpha = 0.7, position = pos, size = 4) +
...
scale_shape_manual(values=c(15, 16, 17, 18)) +
scale_colour_manual(values=c("black", "yellow", "orange", "red2", "grey65", "purple")) +
geom_point(alpha = 0.7, position = pos, size = 3) +
...
plot1 <- ggplot(subset(data_graphing_filtered, Consequence_CBE %in% c("synonymous","UTR", "promoter", NA) ==FALSE & Gene == "JAK1"), aes(x=Amino_Acid_Position_simple_CBE, y= zscore_IFNG_Proliferation_Average_NGG, colour = Consequence_CBE)) + 
...
plot1 <- ggplot(subset(data_graphing_filtered, Consequence_CBE %in% c("synonymous","UTR", "promoter", NA) ==FALSE & Gene == "JAK1"), aes(x=Amino_Acid_Position_simple_CBE, y= zscore_iBE3_9NGN_cl12_ifng_vs_cntrl, colour = Consequence_CBE)) + 
geom_point(position = pos, shape = 16, alpha = 0.80, size =3) +
...
plot1 <- ggplot(subset(data_graphing_filtered, Consequence_ABE %in% c("synonymous","UTR", "promoter", NA) ==FALSE & Gene == "JAK1"), aes(x=Amino_Acid_Position_simple_ABE, y= L2FC_iABE8eNGN_cl12_IFNg_Control, colour = Consequence_ABE)) + 
geom_point(position = pos, shape = 16, alpha = 0.80, size =3) +
...
app_data <- bind_rows(BE3_NGG, BE4max_YE1_NGN, BE3.9max_NGN, ABE8e_NGN, BE3_NGG_IFNG_pathway)
...
annotate("text", y = 1:8, x = -2, label = c("italic(n) == 955", "italic(n) == 217", "italic(n) == 140", "italic(n) == 158", "italic(n) == 55", "italic(n) == 227", "italic(n) == 292", "italic(n) == 550"), parse = TRUE) +
...
annotate("text", y = 1:8, x = -2, label = c("italic(n) == 566", "italic(n) == 334", "italic(n) == 341", "italic(n) == 158", "italic(n) == 55", "italic(n) == 225", "italic(n) == 81", "italic(n) == 859"), parse = TRUE) +
...
annotate("text", y = 1:8, x = -3.5, label = c("italic(n) == 398", "italic(n) == 281", "italic(n) == 311", "italic(n) == 158", "italic(n) == 55", "italic(n) == 85", "italic(n) == 47", "italic(n) == 564"), parse = TRUE) +
...
annotate("text", y = 1:8, x = -3.5, label = c("italic(n) == 134", "italic(n) == 313", "italic(n) == 79", "italic(n) == 158", "italic(n) == 55", "italic(n) == 71", "italic(n) == 17", "italic(n) == 216"), parse = TRUE) +
...
supp_table_5 %>% select(Gene, Amino_Acid_Position) %>%
  distinct() %>%
  count(Gene)
...
supp_table_5 %>% select(Gene, Amino_Acid_Position, predicted_function) %>%
  distinct() %>%
  count(Gene, predicted_function)
...
supp_table_5 %>% 
  mutate(Variant_Recapitulation = ifelse(str_detect(Clinical_Protein_Change, Amino_Acid_Change), TRUE, 
                                         ifelse(str_detect(Amino_Acid_Change, Clinical_Protein_Change), TRUE, FALSE))) %>%
  select(Gene, Amino_Acid_Change, Clinical_Protein_Change, Variant_Recapitulation, editor) %>%
  distinct() %>%
  count(editor, Variant_Recapitulation)
...
#CBE NGN at site with mutation
(18+21)/(18+21+4) *100

#ABE NGN clinical match
12/(12+99+59) *100

#ABE NGN at site with mutation
(12+99)/(12+99+59) *100
...
plot1 <- ggplot(subset(app_data, Consequence %in% c("synonymous","UTR", "promoter", NA) ==FALSE & Gene =="JAK1"), aes(x=Amino_Acid_Position_simple, y= zscore_proliferation, colour = editor, shape = Consequence)) + 
...
scale_colour_manual(values=c("black", "springgreen4", "orange", "royal blue", "red")) +
scale_shape_manual(values=c(19, 1, 6, 5, 4)) +
geom_hline(yintercept=c(0,0),linetype="dotted") +
